...
-type(amqp_method_field_name() ::
       ( consumer_tag | nowait | identifier | consumer_tag
       | arguments | immediate | consumer_tag | content_size
       | exclusive | requeue | delivery_tag | passive
       | nowait | ticket | host | staged_size
       | queue | reply_text | nowait | queue
       | ticket | exchange | message_count | immediate
       | exchange | queue | challenge | immediate
       | result | response | ticket | nowait
       | queue | nowait | prefetch_size | if_unused
       | if_empty | redelivered | nowait | prefetch_count
       | delivery_tag | prefetch_count | exchange | global
       | exchange | routing_key | message_count | integer_1
       | consumer_tag | consume_rate | delivery_tag | consumer_tag
       | durable | ticket | queue | integer_result
       | exchange | ticket | routing_key | operation
       | arguments | queue | no_ack | routing_key
       | integer_4 | exclusive | reply_text | nowait
       | consumer_tag | exchange | routing_key | ticket
       | consumer_tag | exchange | reply_code | class_id
       | channel_max | prefetch_count | no_ack | global
       | routing_key | consumer_tag | frame_max | ticket
       | method_id | ticket | queue | consumer_tag
       | cluster_id | no_local | no_ack | exchange
       | delivery_tag | integer_2 | heartbeat | nowait
       | delivery_tag | queue | consumer_tag | multiple
       | ticket | queue | routing_key | exchange
       | consumer_tag | consumer_tag | routing_key | reply_text
       | details | identifier | mandatory | string_result
       | ticket | reply_code | arguments | mandatory
       | reply_text | class_id | method_id | exchange
       | routing_key | string_2 | version_major | out_of_band
       | mandatory | version_minor | server_properties | nowait
       | mechanisms | active | write | exclusive
       | identifier | requeue | read | operation
       | mechanism | reply_code | response | delivery_tag
       | locale | consumer_tag | prefetch_size | ticket
       | delivery_tag | multiple | exchange | result
       | consumer_tag | type | routing_key | passive
       | durable | integer_3 | auto_delete | internal
       | locales | exclusive | nowait | exchange
       | channel_max | frame_max | heartbeat | delivery_tag
       | ticket | reply_code | exchange | no_local
       | virtual_host | meta_data | if_unused | capabilities
       | requeue | insist | redelivered | message_count
       | content_checksum | known_hosts | redelivered | integer_op
       | string_1 | ticket | client_properties | string_op
       | active | known_hosts | nowait | passive
       | reply_code | exclusive | routing_key | auto_delete
       | nowait | consumer_tag | arguments | reply_text
       | requeue | dtx_identifier | queue | message_count
       | consumer_count | table | no_local | ticket
       | reply_code | active | prefetch_size | ticket
       | global | queue | exchange | realm
